...
function AddReplaceParts({ jobcard_id }) {
  ...
  const validateInputs = () => {
    ...
    const partInputRegex = /^[A-Za-z0-9\s-]+$/;

    if (!partInputRegex.test(partInput)) {
      toast.error('Invalid part input. Please use alphanumeric characters only.');
      return false;
    }

    if (!priceRegex.test(price)) {
      toast.error('Invalid price. Please enter a valid number greater than zero.');
      return false;
    }

    if (quantity <= 0 || !Number.isInteger(Number(quantity))) {
      toast.error('Quantity must be a positive integer greater than zero.');
      return false;
    }

    if (!partInput || !price || !quantity) {
  ...
  return (
    <div className="px-5 pt-4">
      ...
      <div className="mt-8">
                ...
                {partSuggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg z-10">
                    {partSuggestions.map(part => (
                      ...
                      >
                        <div className="text-black">{part.part_id} - {part.name}</div>
                        <div className="text-gray-500">Price: {part.price}</div>
                        <div className="text-gray-500">Available Stock: {part.quantity}</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="bg-gray-100 rounded-lg shadow-md p-4">
          <h2 className="text-lg font-bold text-black">Added Parts Table</h2>
          <table className="w-full mt-4">
            <thead>
              <tr>
                <th className="px-4 py-2 bg-gray-200 text-gray-700">Part Code</th>
