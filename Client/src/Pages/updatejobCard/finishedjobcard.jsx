...
//         try {
//             const response = await axios.get('http://localhost:8800/api/jobcard/getFinishjobcard');
...
//         Swal.fire({
//             title: 'Are you sure?',
//             text: "Do you want to update this job card?",
//             icon: 'warning',
...
//                 navigate('/updateJobCard', { state: { veh_num: row.veh_num, jobcard_id: row.jobcard_id, supervisor: row.u_name } });
//             }
//         });
//     };
...
//         Swal.fire({
//             title: 'Are you sure?',
//             text: "Do you want to update the status of this job card?",
//             icon: 'warning',
//             showCancelButton: true,
//             confirmButtonColor: '#3085d6',
//             cancelButtonColor: '#d33',
...
//             if (result.isConfirmed) {
//                 try {
//                     await axios.put(`http://localhost:8800/api/jobcard/updateStatus`, {
//                         jobcard_id: row.jobcard_id,
//                         status: 'Started'
//                     });
//                     setStatus('Started');
//                     Swal.fire('Updated!', 'Job card status has been updated.', 'success');
//                 } catch (error) {
//                     console.error('Error updating job card status:', error.message);
//                     Swal.fire('Error!', 'Failed to update job card status.', 'error');
//                 }
//             }
//         });
//     };
...
//                 <td className="p-4 text-right text-base">{row.veh_num}</td>
//                 <td className="p-4 text-right text-base">{row.u_name}</td>
//                 <td className="p-4 text-right text-base">{status}</td>
//                 <td className="p-4 text-right text-base">
//                     <button
//                         type="button"
//                         className="bg-lightblue text-white px-4 py-2 rounded-md mr-2"
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    setStatus('Started');
                    Swal.fire('Updated!', 'Job card status has been updated.', 'success').then(() => {
                        navigate('/updateJobCard', { state: { veh_num: row.veh_num, jobcard_id: row.jobcard_id, supervisor: row.u_name } });
                    });
                } catch (error) {
                    console.error('Error updating job card status:', error.message);
            ...
            <tr className="border-b border-gray-300">
                ...
                <td className="p-4 text-right text-base">
                    ...
                    <button
                        type="button"
                        className="bg-lightblue text-white px-4 py-2 rounded-md mr-2"
                        onClick={() => navigate(`/bill`, { state: { jobcard_id: row.jobcard_id } })}
                    >
                        Bill
                    </button>
                </td>
