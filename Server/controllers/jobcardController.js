...
export const addnewcustomer = async (req, res) => {
    ...
    try {
        const query = `INSERT INTO customer_info (contact_number, first_name, last_name, address) VALUES (?, ?, ?, ?)`;
        await new Promise((resolve, reject) => {
            db.query(query, [contact_number, first_name, last_name, address], (err, result) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
...
export const updatevehconnum = async (req, res) => {
    const { veh_num } = req.params;
    const { contact_number } = req.body;

    try {
        const query = `UPDATE vehicle_details SET contact_number = ? WHERE veh_num = ?`;
        await new Promise((resolve, reject) => {
            db.query(query, [contact_number, veh_num], (err, result) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
        ...
        res.status(200).json({ message: 'Vehicle contact number updated successfully' });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'An unexpected error occurred' });
    }
