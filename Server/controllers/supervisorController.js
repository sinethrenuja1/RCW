...
export const updateJobCardStatus = async (req, res) => {
    ...
    console.log("updateJobCardStatus", jobcard_id, status);
...
export const StartedJobCards = async (req, res) => {
    const { user_id } = req.query; // Get user_id from the query parameters
    const query = 'SELECT * FROM job_carddetails WHERE status = "started" AND user_id = ?';
    ...
    try {
        ...
        res.status(200).json({ message: 'Started job cards fetched successfully', data: results });
    } catch (error) {
        console.error('Error fetching started job cards:', error);
        res.status(500).json({ message: 'Internal server error' });
    }
};
